...
# Llama-3.3-70B — Disaggregated Single-Node GAIE Recipe

Deploy **Llama-3.3-70B-Instruct** with prefill/decode disaggregation on a
single 8-GPU node, integrated with
...
# 1. Deploy the Dynamo inference graph (prefill + decode workers + EPP)
kubectl apply -f deploy.yaml -n ${NAMESPACE}

# 2. Wait for the deployment to be ready
kubectl wait --for=condition=Available \
  dynamographdeployment/llama3-70b-disagg \
...
kubectl get pods -n ${NAMESPACE} -l app.kubernetes.io/instance=llama3-70b-disagg
...
| File | Description |
|------|-------------|
| `deploy.yaml` | `DynamoGraphDeployment` with EPP, prefill workers, and decode workers |
| `http-route.yaml` | `HTTPRoute` that directs traffic through the Gateway to the EPP service |

## Notes

- This recipe uses 8 GPUs on a single node: 4 GPUs for prefill (TP4) and
  4 GPUs for decode (TP4).
- The EPP routes requests to prefill workers using KV-overlap scoring, then
  forwards the populated KV cache to the corresponding decode worker via NIXL.
