...
<ParamField path="restart" type="Restart">
  Restart policy for the graph. Change `restart.id` to trigger a restart. For the operational procedure, see [Restart a DGD](../../kubernetes/operations/restart-a-dgd.md).
</ParamField>
...
<ParamField path="status.checkpoints" type="map[string]ComponentCheckpointStatus">
  Per-component checkpoint status, keyed by component name. See the [DynamoCheckpoint API](full-api-reference.mdx#dynamocheckpoint).
